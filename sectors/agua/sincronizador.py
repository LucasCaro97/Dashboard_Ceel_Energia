"""Pipeline mensual de sincronizacion de socios de Agua desde TRYLOGYC."""
...
DEFAULT_INPUT = ROOT / "data" / "agua" / "socios" / "socios_normalizados.csv"

AGUA_CONFIG = SectorSyncConfig(
...
def ejecutar_sync(
    ...
    """Sincroniza socios/medidores/tarifas del sector Agua."""
...
def print_summary(summary: SyncSummary) -> None:
    print_summary_sector(summary, AGUA_CONFIG)


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Sincroniza socios/medidores/tarifas de Agua."
    )
    parser.add_argument(
        "--input",
        default=str(DEFAULT_INPUT),
        help="Ruta del CSV normalizado (default: data/agua/socios/socios_normalizados.csv).",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--export-reportes-csv",
        action="store_true",
        help="Genera CSVs de control en data/agua/reportes_sincro/.",
    )
